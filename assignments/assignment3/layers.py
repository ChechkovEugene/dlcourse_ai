...
class ConvolutionalLayer:
    ...
    def forward(self, X):
        ...
        X_aligned = X_padded[:, :, :, :, np.newaxis]
        W_aligned = self.W.value[np.newaxis, :, :, :, :]
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                X_sel = X_aligned[:,y:y+self.filter_size, x:x+self.filter_size,:]
                out[:,y,x,:] = np.sum(X_sel * W_aligned, axis=(1,2,3)) + self.B.value
    ...
    def forward(self, X):
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                out[:, y, x, :] = np.max(X_slice, axis=(1, 2))
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                mask = (X_slice == np.max(X_slice, (1, 2))[:, np.newaxis, np.newaxis, :])
...
class Flattener:
    ...
    def forward(self, X):
        ...
        return X.reshape(-1, height * width * channels)
